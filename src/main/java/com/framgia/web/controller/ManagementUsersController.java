...
public class ManagementUsersController {
	...
	public ModelAndView referencePage() {
		...
		List<com.framgia.users.model.Permissions> permissionList = 
				managementUsersService.findByAllPermissions();
	...
	public @ResponseBody List<Users> findByCondition(@RequestParam(value = "txtName") String txtName,
		...
		try {
			List<com.framgia.users.model.Users> userList = managementUsersService.findByUsersWithCondition(txtName, txtPermission);
	...
	@RequestMapping(value = "/managementUser/detail/{id}", method = RequestMethod.GET)
	public ModelAndView detailPage(@PathVariable("id") String idUser) {
		...
		return new ModelAndView("managementUserDetail", "user", user);
	}
